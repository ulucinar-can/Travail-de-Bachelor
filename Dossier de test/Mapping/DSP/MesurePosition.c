...
//      Position4 = ADC_pos_4;
     ...
     * --------------------------------------------------------------------- */
    if (state_PIN) // Active l'envoi UNIQUEMENT si le bouton "Start" a ÈtÈ pressÈ
    {
        UartCounter++;
        // Boucle ‡ 25kHz / 10 = Envoi ‡ 2.5 kHz (Une mesure toutes les 400 µs)
        if (UartCounter >= 10)
        {
            UartCounter = 0;

            // Envoi exclusif des 4 positions filtrÈes
            SendFloatAsBinary(Position1, Position2, Position3, Position4);
        }
    }
    else
    {
        UartCounter = 0; // Maintien du compteur ‡ 0 ‡ l'arrÍt
    }
