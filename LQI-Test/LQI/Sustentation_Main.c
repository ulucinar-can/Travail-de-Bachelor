...
float ThX=0, Mx_app=0, err_r=0, th_n=0, thd_n=0, Mx_n=0, dF_roll=0;

// --- Variable pour détecter le vol des inducteur 3 et 4 ---
...
//    Position4 = ((float)(ADC_pos_4)) * GAIN_COR_4 + OFFSET_COR_4;
    ...
    float for_est3_new =                                         AD33_3 * for_est3 + BD3_3 * (fc3f - FP) + L3_3 * err_obs3;
    ...
    if (!SystemOn && (state != STATE_0) && (state != STATE_7))
    {
        state = STATE_7;
    }
    ...
    switch(state)
       {
           ...
           case STATE_4:
               ...
               if(dt_mean >= 200) // dt = 8ms
               {
                   ...
                   if((mean3 <= I_SP105 && mean3 >= I_SP095) && (mean4 <= I_SP105 && mean4 >= I_SP095))
                   {
                       ...
                       // Nouvelle détection avec l'avant qui se lève
                       Position_c3_dec = 0.985f*Position3;
                       Position_c4_dec = 0.985f*Position4;
           ...
           case STATE_5:
               ...
               {
               detect_count++;
                   if(detect_count >= 250)   // condition vraie 10 ms d'affilee
                   {
                       // Update des variables
           ...
           case STATE_6:
               ...
               if(i_store > 7500)   // hold 0.3 s, PUIS rampe douce identique a l'avant
               {
                   if(Position_c3 > DELTA_N) Position_c3 -= 2.5f * 4e-7f; else Position_c3 = DELTA_N;
                   if(Position_c4 > DELTA_N) Position_c4 -= 2.5f * 4e-7f; else Position_c4 = DELTA_N;
               }

               // Timer pour attendre un peu pour  tre sur que le syst me   bien d coller
               if(i_store == I_STORE_2E_DECOLLAGE + 7500)
               {
                   // Reset de la varibale
                   i_store = 0;
    ...
    if(state != STATE_0)
    {
        ...
        // Angle de roulis mesuré : pas de "float", on écrit dans la globale
        ThX = GEO_THX * (Position1 - Position2 + Position3 - Position4);

        // On n'active l'observateur et l'injection QUE si les 4 coins régulent
        if (PosRegFlag1 && PosRegFlag3)
        {
            // Pas de "float" ici non plus : on met à jour les globales
            Mx_app = 0.13f * (fc1f - fc2f + fc3f - fc4f);
            err_r  = ThX - th_est;
            th_n   = th_est  + ROLL_AD12 * thd_est + ROLL_L1 * err_r;
            thd_n  = thd_est + ROLL_AD23 * Mx_est  + ROLL_L2 * err_r;
            Mx_n   = ROLL_AD33 * Mx_est  + ROLL_BD3 * Mx_app + ROLL_L3 * err_r;

            // Mise à jour de l'état de l'observateur (déjà globales)
            th_est  = th_n;
            thd_est = thd_n;
            Mx_est  = Mx_n;

            // Calcul de l'action de correction
            dF_roll = K_ROLL_D * thd_est;
        }
        else
        {
            // Phase de rampe (arrière au sol) : on force l'observateur à suivre sans dériver
            th_est  = ThX;
            thd_est = 0.0f;
            Mx_est  = 0.0f;
            dF_roll = 0.0f;

            // Optionnel mais propre : on remet aussi les variables de calcul à 0
            // pour que la télémétrie soit "propre" quand le bloc est gelé
            Mx_app = 0.0f;
            err_r  = 0.0f;
            th_n   = ThX;
            thd_n  = 0.0f;
