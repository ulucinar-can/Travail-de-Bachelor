 ...
 * FunctionHeader.c
 *
 * Created by : Ulu�inar (2026)
 * Modified by: Ulu�inar (2026)
 *
...
float apply_poly5(float x, const float* coeffs)
{
    ...
    // S�curit� : emp�che l'entrefer de devenir math�matiquement n�gatif
    if (result < 0.0f) {
        result = 0.0f;
    }
...
void Send32FloatsAsCSV(float v[32])
{
    ...
    // Ajoute les 31 premi res valeurs suivies d'une virgule
    for(k = 0; k < 31; k++) {
        i += ftoa(v[k], &str[i], 4);
        str[i++] = ',';
    }

    // Ajoute la 32 me valeur sans virgule, suivie d'un retour   la ligne
    i += ftoa(v[31], &str[i], 4);
    str[i++] = '\n';
