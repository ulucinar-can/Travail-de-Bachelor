...
#define LOG_SIZE 2500       // Taille des tableaux (2500 points = ~25s � 100Hz)
...
//    Position4 = ((float)(ADC_pos_4)) * GAIN_COR_4 + OFFSET_COR_4;
    ...
    // D�tection d'un appui sur le bouton (flanc montant de ButtonS2)
    if (ButtonS2 && !previous_button_state) {
        // Le bouton vient d'�tre activ� : On d�marre ou on r�initialise l'enregistrement
        is_logging = true;
        log_index = 0;           // On recommence depuis le d�but du tableau
        downsample_counter = 0;
    }
    // D�tection d'un deuxi�me appui (flanc descendant de ButtonS2)
    else if (!ButtonS2 && previous_button_state) {
        // Le bouton vient d'�tre d�sactiv� : On arr�te l'enregistrement
        is_logging = false;
    }

    previous_button_state = ButtonS2; // Mise � jour de l'�tat pour le prochain tick
    ...
    if (is_logging && (log_index < LOG_SIZE)) {
        ...
        if (downsample_counter >= DOWNSAMPLE_LIMIT) {
            downsample_counter = 0; // Reset du sous-�chantillonneur
            ...
            log_Duty[log_index]  = duty_table[0]; // Ou dutyCycle1 si tu pr�f�res
            log_State[log_index] = state;

            // Tu peux rajouter tes autres variables ici (erreur, vitesses, etc.)

            log_index++; // On passe � la case suivante pour le prochain coup
        }
    }
    else if (log_index >= LOG_SIZE) {
        // S�curit� : si le tableau est plein, on arr�te automatiquement d'enregistrer
        is_logging = false;
    }
}
